...
updated: 2020-09-20T05:03:27-07:00
tocDepth: 3
description: >
...
Until recently, I didn't know about the [`<datalist>`][datalist] element until I
read [Peter Bengtsson's article][peter]. [According to MDN][datalist]:
...
1. We initialize `mruTerms` when the component is first rendered. In a real app,
   this can be fetched from a service, upstream app state, or `localStorage`.
1. `<option>` elements of the `<datalist>` are generated from `mruTerms`. Notice
   the entire `<datalist>` component is replaced every time `term` changes since
   we bind it to the `key` prop.
   - You can think of `mruTerms` as a detail of `term`, since every time `term`
     changes, we are generating a new array of associated `mruTerms`.
...
1. We move most of the business logic into a custom hook, simplifying the actual
   component.
...
1. First, suggestions that begin with the current search term, e.g.
   "<mark>cr</mark>anial", "<mark>cr</mark>eamery".
1. Next, suggestions that contain words that start with the current search, e.g.
   "heavy <mark>cr</mark>ate", "ice <mark>cr</mark>eam".
1. Finally, suggestions that contain the search term, anywhere, e.g.
   "dis<mark>cr</mark>eet", "s<mark>cr</mark>ibe".
...
context data: `draft` and `sortedMruTerms`:
...
1. Add action to handle `updateDraft`, which is called each time the search term
   changes.
1. `updateTerm` no longer requires a payload, since we can calculate new context
   values based on `draft`.
1. Make some tweaks to the custom hook (I trust you can figure this part out on
   your own).
...
test the utility functions independently. Composition FTW 🙌.
...
[Weston Thayer][] and I had a discussion about accessibility for data list. He
pointed out the following issues and resources for further investigation.
...
Combobox][reach-ui-combobox]. Also [read 24 Accessibility's "&lt;select&gt; Your
Poision"][select-your-poision] article for in-depth discussion of why this is a
...
<Pronoun client:visible />

## Takeaways

- For combo boxes, try the built-in `<datalist>` element first.
- You can dynamically generate the datalist options based on events.
- Abstract out as much logic as possible from your components for composability,
  testability, and agility.
